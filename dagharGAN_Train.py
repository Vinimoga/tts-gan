...
def parse_args():
    ...
    parser.add_argument('--log_path', type=str, default='logs/retest_0_batchsize_16/')
...
max_iter = 137694  #300 épocas: 137694  50 épocas: 27538
checkpoint_number = 4
seq_len = 60
channels = 6

log_dir = args.log_path      #'Projetos/tts-gan/logs/gen_batch_size_64' 
path = '../DAGHAR_split_25_10_all/train/'            #'Projetos/DAGHAR_split_25_10_all/train/' 
data_path = path + 'data/'
classes = os.listdir(data_path)
label_path = [path + 'label/' + s.replace('DAGHAR', 'Label') for s in classes]


default_string = f"CUDA_VISIBLE_DEVICES=1 python train_GAN_dahar.py -gen_bs 64 -dis_bs 64 \
                ...
                --beta2 0.999 --phi 1 --batch_size 64 --num_eval_imgs 50000  \
                --init_type xavier_uniform --n_critic 1 --val_freq 20 --print_freq 1000  \
                --grow_steps 0 0 --fade_in 0 --patch_size 2 --ema_kimg 500 --ema_warmup 0.1  \
                --ema 0.9999 --diff_aug translation,cutout,color --random_seed {args.seed} "
...
for i, class_name in enumerate([s for s in classes]):
    ...
    command = f'{default_string}' + f' --class_name {c}' + f' --seq_len {seq_len}'\
