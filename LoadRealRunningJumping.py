...
class Running_Or_Jumping(Dataset):
    def __init__(self, 
        ...
        data_mode = 'Train',
        data_path = os.path.join("./UniMiB-SHAR")):
        ...
        print(data_path)
        if (not os.path.isfile(data_path + '.zip')):
            ...
            self.download_url('https://www.dropbox.com/s/raw/x2fpfqj0bpf8ep6/UniMiB-SHAR.zip', data_path + '.zip')
        if (not os.path.isdir(data_path)):
            shutil.unpack_archive(data_path + '.zip','.','zip')
        #Convert .mat files to numpy ndarrays
        path_in = data_path + '/data'
