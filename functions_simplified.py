    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        if args.loss == "hinge":
            ...
            real_label = torch.full(
                (imgs.shape[0],), 0.9, dtype=torch.float, device=real_imgs.get_device()
            )
            fake_label = torch.full(
                (imgs.shape[0],), 0.1, dtype=torch.float, device=real_imgs.get_device()
            )
...
def train(
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
       ...
       # Ensure imgs is a tensor, and convert it explicitly to float32 on CPU
        ...
        if (iter_idx + 1) % args.accumulated_times == 0:
            torch.nn.utils.clip_grad_norm_(dis_net.parameters(), 5.0)
            save_state_of_model(dis_net, dis_optimizer, real_imgs, z, d_loss, f'dis_net_pre_step_{i}.pth')
            dis_optimizer.step() 
            dis_optimizer.zero_grad()
            save_state_of_model(dis_net, dis_optimizer, real_imgs, z, d_loss, f'dis_net_post_step_{i}.pth')   
        ...
        save_state_of_model(gen_net, gen_optimizer, real_imgs, gen_z, g_loss,f'gen_net_pre_step_{i}.pth')
        gen_optimizer.step()
        gen_optimizer.zero_grad()
        save_state_of_model(gen_net, gen_optimizer, real_imgs, gen_z, g_loss, f'gen_net_post_step_{i}.pth')
        i = i+1
        if i == 367:
            print(i)
        elif i == 368:
        ...
        #     ema_nimg = min(ema_nimg, cur_nimg * args.ema_warmup)
        #     ema_beta = 0.5 ** (
        #         float(args.dis_batch_size * args.world_size) / max(ema_nimg, 1e-8)
        #     )
        # else:
        #     ema_beta = args.ema
...
def save_samples(
    ...
    with torch.no_grad():
        ...
        save_image(
            sample_imgs,
            f"./samples/{args.exp_name}/sampled_images_{epoch}.png",
            nrow=10,
            normalize=True,
            scale_each=True,
        )
...
def save_state_of_model(model, optimizer, batch, z, loss,final_part_of_path):
    """
    ...
    os.makedirs(f'/workspaces/vinicius.garcia/Projetos/tts-gan/debbuging/', exist_ok=True)
    torch.save({
    ...
    f'/workspaces/vinicius.garcia/Projetos/tts-gan/debbuging/{final_part_of_path}')
